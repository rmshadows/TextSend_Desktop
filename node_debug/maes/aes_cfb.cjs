 ...
 * @returns 
 */
function cfbCipher(SecuritykeyStr, initVectorStr, pwdLength = 32){
    try {
        // 转字符串为byte[] padding
 ...
 * @returns 
 */
function cfbDecipher(SecuritykeyStr, initVectorStr, pwdLength = 32){
    try {
        // 转字符串为byte[] padding
        let bSecuritykeyStr = tools.keyPadding(SecuritykeyStr, pwdLength);
        let binitVectorStr = tools.keyPadding(initVectorStr, 16);
 ...
 * @returns 
 */
function tEncrypt(message, key, iv, length = 32){
    try {
 ...
 * @returns 
 */
function tDecrypt(hexmessage, key, iv, length = 32){
    try {
        // the decipher function
