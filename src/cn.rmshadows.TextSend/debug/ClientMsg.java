...
public class ClientMsg {
    // 服务器消息自带的ID
    final static String SERVER_ID = "-200";
    ...
    public void run() {
        try (Scanner sc = new Scanner(System.in);) {
            while (true) {
                String string = null;
                string = sc.nextLine();
                Message m = new Message(ClientMsg.id, string, ClientMsg.MSG_LEN, null);
                oos.writeObject(m);
                oos.flush();
            }
        } catch (Exception e) {
...
class ClientR implements Runnable {
    ...
    private String decryptMsgToString(Message m) {
        ...
        for (String s : m.getData()) {
            str += MessageCrypto.tsDecryptString(s);
        }
        return str;
    }
    ...
    private static void msgFeedBack(ObjectOutputStream out) throws IOException {
        ...
        out.writeObject(new Message(ClientMsg.id, null, ClientMsg.MSG_LEN, ClientMsg.FB_MSG));
    }
}
