...
- 「浏览」左键 Java 对话框（可把文件拖进对话框，打开该文件所在文件夹）；右键用系统文件选择框。Shift+左键发 **1 个文件夹**（按相对路径在对面重建，不是同步）。
- 对面落到系统「下载/TextSend」，不弹保存框；重名自动 `name (1).ext`。
- 发送中仍可往列表加文件，会接着发。底栏有进度、实时速度和「取消」（发/收都能停）。
- 发文件夹时进度显示 `3/40` 和第几个文件的百分比。
- 断线可续传（认文件头+尾哈希；对不上则从头）。点取消会丢掉半截。
- 多台已连接设备会全发，超过 1 台时提示数量；同一文件并行发给各端。
- 现有 Android / uTools 还不支持文件，发给它们会立刻提示对端不支持。
...
连接好手机后，打开一个Word文档，鼠标在Word文本输入的地方点一下，保证文本输入区域是焦点。这样手机发过来的文字会直接打在Word文档中。如果不小心失去焦点，没输入成功。你只需要右击粘贴就是，因为手机发送的文字也是存在电脑剪贴板上的。手机端目前还不能收文件。
